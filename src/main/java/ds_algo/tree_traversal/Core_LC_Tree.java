   ...
   */
    ...
    */
    ...
    public TreeNode sortedArrayToBST(int[] nums) {
        return helperSortedArrayToBST(0, nums.length-1, nums);

    }

    private TreeNode helperSortedArrayToBST(int left, int right, int[] nums){
        if (left > right) return null;
        int mid = left + (right - left) /2;

        TreeNode root = new TreeNode(nums[mid]);
        root.left = helperSortedArrayToBST(left, mid - 1, nums);
        root.right = helperSortedArrayToBST(mid + 1, right, nums);
        return root;
    }
}
